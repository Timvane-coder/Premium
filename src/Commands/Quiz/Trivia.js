    ...
    async execute(sock, m) {
        try {
            if (!currentQuestion) { // Start a new game
                ...
                await buddy.reply(m, `${emojis.thinking} Trivia Time!\n\n${currentQuestion.question}\n\nOptions:\n${currentQuestion.options.map((opt, i) => `${i+1}. ${opt}`).join('\n')}`);

                // Set a timeout for the question
                setTimeout(async () => {
                    if (currentQuestion) {
                        await buddy.reply(m, `${emojis.error} Time's up! The correct answer was: ${currentQuestion.answer}`);
                ...
                if (isNaN(userAnswer) || userAnswer < 0 || userAnswer >= currentQuestion.options.length) {
                    return await buddy.reply(m, `${emojis.error} Invalid answer. Please choose a number from 1 to ${currentQuestion.options.length}.`);
                }

                if (currentQuestion.options[userAnswer] === currentQuestion.answer) {
                    await buddy.reply(m, `${emojis.success} Correct!\n\n${currentQuestion.fact}`);
                } else {
                    await buddy.reply(m, `${emojis.error} Incorrect! The correct answer was: ${currentQuestion.answer}\n\n${currentQuestion.fact}`);
                }
            ...
            await buddy.reply(m, `${emojis.error} Error: ${error.message}`);
        }
    }
};
