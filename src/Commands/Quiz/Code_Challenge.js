        ...
        try {
            ...
            if (type === "codechallenge") {
                ...
                if (!challenges[language] || !challenges[language][level]) {
                    return await buddy.reply(m, "Invalid level or language. Please choose from: easy, medium, hard and javascript, python, java, etc. Example `/codechallenge easy javascript`");
                }

                const challenge = getRandomItem(challenges[language][level]);
                await buddy.reply(m, `${emojis.challenge} **Coding Challenge:**\n\n${challenge.question}\n\nType your solution in a new message. (Use /giveup if you're stuck)`);

                const sentMsg = await buddy.reply(m, `${emojis.challenge} **Coding Challenge:**\n\n${challenge.question}\n\nType your solution in a new message. (Use /giveup if you're stuck)`);
                const response = await buddy.getResponseText(m, sentMsg, 15000); // 60 seconds timeout

                if (!response) {
                    return await buddy.reply(m, "Time's up! ⏰ The correct answer was:\n\n`" + challenge.solution + "`");
                }
                if (response.response.startsWith("/giveup")) {
                    return await buddy.reply(m, `${emojis.hint} The solution was:\n\n\`\`\`${challenge.solution}\`\`\``);
                }
                ...
                if (isCorrect) {
                    await buddy.reply(m, `${emojis.correct} Correct! Great job!`);
                } else {
                    await buddy.reply(m, `${emojis.incorrect} Incorrect. Try again or use \`/giveup\` for the solution.`);
                }

            } else if (type === "quiz") {
                if (!quizzes[category] || !quizzes[category][level]) {
                    return await buddy.reply(m, "Invalid level or category. Please choose from: easy, medium, hard and general, science, history, etc. Example `/quiz easy general`");
                }

                const quiz = getRandomItem(quizzes[category][level]);
                const optionsText = quiz.options.map((option, index) => `${index + 1}. ${option}`).join('\n');
                await buddy.reply(m, `${emojis.quiz} **Quiz:**\n\n${quiz.question}\n\n${optionsText}\n\nReply with the number of the correct answer.`);

                const sentMsg = await buddy.reply(m, `${emojis.quiz} **Quiz:**\n\n${quiz.question}\n\n${optionsText}\n\nReply with the number of the correct answer.`);
                const response = await buddy.getResponseText(m, sentMsg, 30000); // 30 seconds timeout

                if (!response) {
                    return await buddy.reply(m, "Time's up! ⏰ The correct answer was: " + quiz.answer);
                }
                ...
                if (userAnswer === quiz.answer) {
                    ...
                    await buddy.reply(m, `${emojis.incorrect} Incorrect. The correct answer was: ${quiz.answer}`);
                }

            } else {
                await buddy.reply(m, "Please specify a valid command type: `codechallenge` or `quiz`. Example `/codechallenge easy javascript` or `/quiz easy general`.");
            }
        } catch (error) {
            await buddy.reply(m, "❌ An error occurred while processing the command.");
