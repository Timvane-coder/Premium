...
module.exports = {
    ...
    async execute(sock, m, args) {
        ...
        switch (action) {
            ...
            default:
                return buddy.reply(m, `${emojis.error} Invalid action. Use: .ludo <start|join|roll|status>`);
        }
    }
};
...
function startGame(sock, m, groupId, sender) {
    if (games[groupId]) {
        return buddy.reply(m, `${emojis.error} A game is already in progress in this group.`);
    }
    ...
    return buddy.reply(m, `${emojis.success} Ludo game started! Use .ludo join to join the game.`);
}

function joinGame(sock, m, groupId, sender) {
    if (!games[groupId]) {
        return buddy.reply(m, `${emojis.error} No game in progress. Use .ludo start to begin a new game.`);
    }

    if (games[groupId].started) {
        return buddy.reply(m, `${emojis.error} Game already in progress. Wait for the next game.`);
    }

    if (games[groupId].players.some(p => p.id === sender)) {
        return buddy.reply(m, `${emojis.error} You've already joined the game.`);
    }

    if (games[groupId].players.length >= 4) {
        return buddy.reply(m, `${emojis.error} Game is full. Maximum 4 players allowed.`);
    }
    ...
    if (games[groupId].players.length === 4) {
        ...
        return buddy.reply(m, `${emojis.success} You joined the game. The game is now full and will begin! Use .ludo roll to take your turn.`);
    }

    return buddy.reply(m, `${emojis.success} You joined the game. Waiting for more players...`);
}
...
function rollDice(sock, m, groupId, sender) {
    ...
    if (currentPlayer.id !== sender) {
        return buddy.reply(m, `${emojis.error} It's not your turn.`);
    }
...
function gameStatus(sock, m, groupId) {
    ...
    return buddy.reply(m, `Current game status:\n${status}\n\nCurrent turn: ${currentPlayer}`);
}