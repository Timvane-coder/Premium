...
module.exports = {
    ...
    async execute(sock, m, args) {
        ...
        switch (action) {
            ...
            default:
                return buddy.reply(m, `${emojis.error} Invalid action. Use: .wm <start|guess|hint|status>`);
        }
    }
};
...
async function startGame(sock, m, groupId, sender) {
    ...
    try {
        ...
        return buddy.reply(m, `${emojis.success} Word Mastermind started! The secret word has ${word.length} letters. Use .wm guess <word> to make a guess.`);
    } catch (error) {
...
async function makeGuess(sock, m, groupId, sender, guess) {
    if (!games[groupId]) {
        return buddy.reply(m, `${emojis.error} No game in progress. Use .wm start to begin a new game.`);
    }

    const game = games[groupId];
    if (!guess || guess.length !== game.word.length) {
        return buddy.reply(m, `${emojis.error} Invalid guess. The word has ${game.word.length} letters.`);
    }
...
function giveHint(sock, m, groupId) {
    ...
    return buddy.reply(m, `${emojis.letter} Hint: ${hint}\nHints remaining: ${game.hints}`);
}
