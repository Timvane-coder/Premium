...
module.exports = {
    ...
    async execute(sock, m, args) {
        if (!args[0]) {
            return buddy.reply(m, `${emojis.error} Please provide a MediaFire link. Usage: .mediafire <url>`);
        }

        const url = args[0];
        await buddy.react(m, emojis.download);
        ...
        try {
            ...
            if (!response.data.status) {
                return buddy.reply(m, `${emojis.error} Failed to fetch file information. Please check your link and try again.`);
            }
            ...
            if (fileSizeInBytes > MAX_DOWNLOAD_SIZE) {
                return buddy.reply(m, `${emojis.warning} File size (${fileData.size}) exceeds the maximum limit of 100 MB.`);
            }

            const infoMessage = await buddy.reply(m, `${emojis.file} File: ${fileName}\n${emojis.size} Size: ${fileData.size}\n\n${emojis.progress} Starting download...`);
            
            const downloadPath = path.join('./temp', fileName);
            await downloadFile(fileData.link, downloadPath, m, infoMessage);

            await buddy.sendFile(m, downloadPath, fileName, `${emojis.success} Here's your file!`, null);
            fs.unlinkSync(downloadPath); // Clean up the downloaded file

        } catch (error) {
            console.error("Error in MediaFire download:", error);
            await buddy.reply(m, `${emojis.error} An error occurred while processing your request. Please try again later.`);
        }
    }
};
...
async function updateProgressMessage(m, infoMessage, downloaded, total) {
    ...
    await buddy.editMsg(m, infoMessage, updatedMessage);
}
