...
module.exports = {
    ...
    async execute(sock, m, args) {
        try {
            ...
            if (!mentionedJid) {
                return await buddy.reply(m, `${emojis.error} Please mention a user to perform the action on.`);
            }

            switch (subcommand) {
                case 'promote':
                    await sock.groupParticipantsUpdate(m.key.remoteJid, [mentionedJid], "promote");
                    await buddy.reply(m, `${emojis.promote} User promoted to admin.`);
                    break;

                case 'demote':
                    await sock.groupParticipantsUpdate(m.key.remoteJid, [mentionedJid], "demote");
                    await buddy.reply(m, `${emojis.demote} User demoted to member.`);
                    break;

                case 'kick':
                    await sock.groupParticipantsUpdate(m.key.remoteJid, [mentionedJid], "remove");
                    await buddy.reply(m, `${emojis.kick} User removed from the group.`);
                    break;

                case 'link':
                    const code = await sock.groupInviteCode(m.key.remoteJid);
                    const inviteLink = `https://chat.whatsapp.com/${code}`;
                    await buddy.reply(m, `${emojis.link} Group Invite Link: ${inviteLink}`);
                    break;

                default:
                    await buddy.reply(m, `${emojis.error} Invalid subcommand. Use promote, demote, kick, or link.`);
            }
        } catch (error) {
            console.error('Error in group command:', error);
            await buddy.reply(m, `${emojis.error} Error performing the action.`);
        }
    }
};
